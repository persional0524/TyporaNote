...
用`Python`写命令行程序的时候，单一的输出颜色太单调。其实我们可以加些色彩，比如用红色表示警告，绿色表示结果正常等。这里，对`Windows`下`CMD`输出彩色进行完整的讲解和易用的代码函数库展示。

先上效果图:

![img](Python3改变cmd（命令行）输出颜色.assets/70.png)
