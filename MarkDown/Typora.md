...
输入`$$`，然后按下`Enter`键将触发一个接收*Tex/LaTeX*源码的输入范围。例如： **V**1×**V**2=∣∣∣∣∣∣**i**∂X∂u∂X∂v**j**∂Y∂u∂Y∂v**k**00∣∣∣∣∣∣V1×V2=|ijk∂X∂u∂Y∂u0∂X∂v∂Y∂v0| 在Markdown源代码文件中，数学公式是被`$$`标记的*LaTeX*表达式：
...
Typora 支持[YAML Front Matter](http://jekyllrb.com/docs/frontmatter/)。输入`---`在文章的顶端然后按下`Enter`键就会采用或者从菜单中插入一个元数据块。
...
Typora支持[sequence](https://bramp.github.io/js-sequence-diagrams/), [flowchart](http://flowchart.js.org/) 和 [mermaid](https://knsv.github.io/mermaid/#mermaid)，可以在设置中启用此功能。

详情请看[document](http://support.typora.io/Draw-Diagrams-With-Markdown/)
...
Ctrl(On Mac：Command) + Click[This link](https://renshuo555.github.io/2017/04/28/markdown-typora/#块元素)会跳转到块元素标题的位置 ，要查看如何写入，请移动光标或单击该链接，按⌘键将元素展开为markdown源。
...
- 图片Image: `<img src="http://www.w3.org/html/logo/img/mark-word-icon.png" width="200px" />` (And `width`, `height` attribute in HTML tag, and `width`, `height`, `zoom` style in `style` attribute will be applied.)
- 注释Comments: `<!-- This is some comments -->`
...
![img](Typora.assets/inline-math.gif)
